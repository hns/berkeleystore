...
function Store(location, options) {
    ...
    function Table(type) {
        ...
        // makesure database table is initialized in transaction
        self.beginTransaction();
        try {
            db = openDatabase(env, dbname, type);
            meta = openDatabase(env, dbname, type + '_meta');
            idgen = openSequence(meta, type);
            openRegisteredIndexes();
        ...
        function openRegisteredIndexes() {
            var data = new DatabaseEntry();
            var result = meta.get(self.getTransaction(), stringToEntry('indexes'), data, null);
            if (result == SUCCESS) {
                var indexarray = JSON.parse(entryToString(data));
                for each (var field in indexarray) {
                    indexes[field] = openIndex(db, dbname, type, field);
                }
            }
        }
        ...
        function getIndex(field) {
            ...
            if (!index) {
                ...
                var result = meta.put(self.getTransaction(),
                        stringToEntry('indexes'),
                        stringToEntry(JSON.stringify(indexarray)));
                if (result == SUCCESS) {
                    index = indexes[field] = openIndex(db, dbname, type, field);
                }
            }
            return index;
        }
    ...
    function openDatabase(env, location, name, options) {
        ...
        return env.openDatabase(self.getTransaction(), location, name, dbconf);
    }

    function openIndex(db, location, type, field, options) {
        ...
        return db.environment.openSecondaryDatabase(self.getTransaction(), location, name, db, dbconf);
    }
    ...
    function openSequence(db, name, options) {
        ...
        return db.openSequence(self.getTransaction(), stringToEntry(name), seqconf);
    }
