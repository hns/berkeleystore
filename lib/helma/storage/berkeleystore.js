        ...
        function QueryImpl() {
            ...
            this.addFilter = function(name, operator, value) {
                ...
                filters.push({
                    initIndexKey: function(key) {
                       propertyToIndexKey(name, value, key, operator >= LESS_THAN);
                    },
            ...
            this.select = function(property) {
                ...
                for each (var filter in filters) {
                    ...
                    var status = filter.initCursor(cursor, key, pkey, data);
                    if (status == SUCCESS) {
                        while(status == SUCCESS) {
                            var s = filter.checkProperty(indexKeyToProperty(key));
                            if (s) {
                                var id = entryToNumber(pkey);
                                results[id] = results[id] + 1 || 1;
                            } else if (s === false) {
                                break;
                            }
                ...
                for (var i in results) {
                    if (results[i] === filters.length) {
                        var s = new Storable(type, createKey(type, i));
            ...
            createSecondaryKeys: function(secondary, primaryKey, primaryData, results) {
                ...
                for (var prop in obj) {
                    ...
                    if (isIndexableProperty(prop, value)) {
                        print("CREATING INDEX FOR ", prop, ", ", value);
