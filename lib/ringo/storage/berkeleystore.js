addToClasspath('/usr/share/java/libdb4.7-java.jar');
...
function Store(location, options) {
    ...
    function Table(type) {
        ...
        this.retrieveAll = function() {
            ...
            var cursor = db.openCursor(self.getTransaction(), null);
            try {
        ...
        function QueryImpl() {
                ...
                filters.push({
                    ...
                    getWeight: function() {
                        ...
                        if (isNaN(this.weight)) {
                            ...
                            var count = this.total = stats.getNumData();
                    ...
                    getResult: function(previousResult, loadEntities, entitiesLoaded) {
                        ...
                        var index = indexes[name];
                        var cursor = index.openCursor(self.getTransaction(), null);
                        try {
            ...
            this.select = function(property) {
                ...
                var entitiesLoaded = false,
                    ...
                    countEquals = 0,
                    result;
                while (countEquals < filters.length && filters[countEquals].getOperator() == EQUAL) {
                    countEquals++;
                }
                if (countEquals > 1 && countEquals == filters.length) {
                    ...
                    try {
                        for each (let filter in filters) {
                            var cursor = filter.getIndex().openCursor(self.getTransaction(), null);
