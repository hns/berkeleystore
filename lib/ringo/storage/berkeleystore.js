...
function Store(location, options) {
            ...
            this.select = function(property) {
                ...
                if (countEquals > 1 && countEquals == filters.length) {
                    ...
                    filters.reduceRight(function(previous, current) {
                        current.remainingWeight = previous + current.getWeight();
                        return current.remainingWeight;
                    }, 0);
                ...
                if (property == '_id') {
                    ...
                    for (let [id, value] in result)
                        list[list.length] = create(type, createKey(type, id), entitiesLoaded ? value : null);
                }
                return list;
            };
        }
    ...
    function openIndex(db, tx, location, type, field, options) {
        ...
        applyOptions(dbconf, options || {
            ...
            type: DatabaseType.BTREE,
            immutableSecondaryKey: true, // avoids repeated calls to multiKeyCreator
